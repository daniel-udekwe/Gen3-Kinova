...
class messageConverter:
    ...
    def __init__(self):
    	...
    	self.base_feedback_sub = rospy.Subscriber("/my_gen3/base_feedback", BaseCyclic_Feedback, self.base_feedback_callback)
        self.base_feedback_boundary_sub = rospy.Subscriber("/my_gen3/base_feedback", BaseCyclic_Feedback, self.base_feedback_boundary_callback)
    	self.Twist_pub = rospy.Publisher("/my_gen3/end_effector_pose", Twist, queue_size=1)
    	self.Kinova_Position = rospy.Publisher("/my_gen3/end_effector_pose", Twist, queue_size=1)
        self.Kinova_Velocity = rospy.Publisher("KinovaAR/end_effector_velocity", Twist, queue_size=1)

        self.KinovaAR_velocity_correction = rospy.Publisher("/my_gen3/in/cartesian_velocity_mod", Twist, queue_size=1)
